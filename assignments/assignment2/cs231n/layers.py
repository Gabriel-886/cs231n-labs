...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    layernorm = bn_param.get('layernorm',0.0)
...
def batchnorm_backward(dout, cache):
    ...
    # Referencing the original paper (http
    #
    #
    #
    #s://arxiv.org/abs/1502.03167)       #
    # might prove to be helpful.                                              #
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    dbeta = dout.sum(axis=int(cache['axis']))
    dgamma = np.sum(dout * cache['z'], int(cache['axis']))
...
def batchnorm_backward_alt(dout, cache):
    ...
    dbeta = dout.sum(axis=int(cache['axis']))
    dgamma = np.sum(dout * cache['z'], axis=int(cache['axis']))
        ...
        for f in range(int(F)):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    receptive_field = x_pad[n, :, vert_start:vert_end, horiz_start:horiz_end] #感受野范围
        ...
        for f in range(F):
            for i in range(H):
                for j in range(W):
                    ...
                    # 提取感受野和对应的梯度
                    receptive_field = x_pad[n, :, vert_start:vert_end, horiz_start:horiz_end]
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # 在池化区域内找到最大值
                    out[n, c, i, j] = np.max(x[n, c, vert_start:vert_end, horiz_start:horiz_end])
