...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        """
        self.normalization = normalization #是否有归一化
        self.use_dropout = dropout_keep_ratio != 1 #若为0，则每一个relu层后紧接一个dropout
        self.reg = reg                            #常量
        self.num_layers = 1 + len(hidden_dims) #神经网路层数
        self.dtype = dtype
        self.params = {}#参数集合
        ...
        if self.normalization != None:
            # batch/layer norm parameters 用到了归一化
...
def affine_norm_relu_forward(x, w, b, gamma, beta, bn_param, normalization, dropout, do_param):
    bn_cache = None
    do_cache = None #不一定会用到，但要先设置成None
